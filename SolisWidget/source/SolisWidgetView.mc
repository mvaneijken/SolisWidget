    ...
    function makeReq() {
        ...
        if(uid.toString().length() <3 ){
            ...
            if(Usr.toString().length == 0 || Pwrd.toString().length == 0){
                ...
                WatchUi.requestUpdate();
                var url =  BaseUrl+"/v/ap.2.0/cust/user/login?user_id=" + Usr + "&user_pass=" + Pwrd + "&terminate=android&push_sn=9601b3865bdabf6ea01a7318ac5cc749&timezone=1&lan=en&country=CN&cust=659";
                var options = {
